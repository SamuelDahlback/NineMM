...
import os
import sys
from NineMM-main.game_engine.UU-game-copy.game_engine.utils.ai_simulation import *
from source.client import *
from source.interface import *
from source.server import *
# Classes---------------------------------------------------------------------------------------------------------------
class Player:
    def __init__(self, name, figure, pieces, nextTurn, userID):
...
def create_new_game():
    ...
    print("\n\tYou will now begin the game by placing " +
          str(MAX_NUMBER_OF_PIECES_PER_PLAYER) + " pieces each")
    place_piece()
        ...
        for player in PLAYERS:
            if player.nextTurn is True:
                while True:
                    print_board()
                    x = get_input(
                        str(" " + player.name + " place a piece on an empty node (A1-G7): "))
                    if x is not None:
...
def get_valid_actions(player,stage):
    empty_nodes = []
    for node in NODE_LIST:
        if node.coord.empty:
            empty_nodes.append(node)

    moves = game.get_available_actions(player)

    if stage == "stage_1" or stage == "stage_3":
        return empty_nodes
    elif stage == "stage_2":
        return moves
...
def get_formed_rows_for_player(player):
    # how many rows a player have in the nodes
    pass

FAVORABLE_POSITIONS = [
        5, 12, 15, 
        ]
LESS_FAVORABLE_POSITIONS = [
        8, 11
        ]
def get_number_of_pieces_in_favorable_position(player):
    # len(player.pieces *intersection* FAVORABLE_POSITIONS)
    pass
def get_number_of_pieces_in_less_favorable_position(player):
    # player.pieces *intersection* LESS_FAVORABLE_POSITIONS
    pass
...
def rules(ACTIVE_GAME):
    ...
    print()
    print(
        "\t\t 2. Place pieces on the board using the coordinate system [A1-G7]. The player will put one piece")
    print("\t\t each turn and will place a total of 9 pieces on the board. When the player has put three pieces")
    print(
        "\t\t in a trio the player will be able to move a piece using the coordinate system [A1-G7]. The player")
    ...
    while True:
        try:
            choice = int(
                input("\t\t\t\t  Enter your 1 to go back to the Menu: "))
            break
        except ValueError:
            print("\t\t\tNot a valid option. Try Again!")
...
NUMBER_OF_NODES = 24
PLAYERS = [Player("Black", "X", [], False,"player1"), Player("White", "O", [], False,"player2")]
NODE_LIST = []
MENU_SHORTCUT = "M"
...
#print_title()
#menu(False)
get_valid_actions("player1","stage_2")
